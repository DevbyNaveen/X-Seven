...
def get_task_db_session():
    """Context manager for Supabase client in Celery tasks."""
    supabase = get_supabase_client()
    try:
        yield supabase
    except Exception as e:
        logger.error(f"Supabase client error in task: {e}")
        raise
...
def setup_periodic_tasks(sender, **kwargs):
    ...
    # Clean up expired orders every 30 minutes (now handled by Temporal)
